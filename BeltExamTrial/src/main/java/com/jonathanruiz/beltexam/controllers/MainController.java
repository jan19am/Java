...
import com.jonathanruiz.beltexam.services.PlayerService;
...
//	Team Details Page
	...
	@GetMapping("/teams/{id}")
    public String showPage(@ModelAttribute("player") Player player, @PathVariable("id") Long id, Model model, HttpSession session) {
    	if(session.getAttribute("userId") == null) {
    		return "redirect:/";
    	}
    ...
    public String createPlayer(@Valid @ModelAttribute("player") Player player, BindingResult result, @PathVariable("id") Long id, Model model) {
